"""生成 VoxCeleb2 的 s1 / mix_clean scp 列表。

用法:
    python get_vox2_list_621.py --split train_scale --output s1
    python get_vox2_list_621.py --split test_scale --output mix_clean
"""
import argparse
...
SPLITS = {
    "train_scale": {
        ...
        "mix_dir": "mix_dir_train_scale",
        "s1_dir": "s1_dir_train_scale",
        "s1_scp": "s1_scp_train_scale",
        "mix_clean_scp": "mix_clean_scp_train_scale",
    },
    "test_scale": {
        "mix_csv": "mix_csv_test_scale",
        "mix_dir": "mix_dir_test_scale",
        "s1_dir": "s1_dir_test_scale",
        "s1_scp": "s1_scp_test_scale",
        "mix_clean_scp": "mix_clean_scp_test_scale",
    },
}


def build_lines(mix_csv, mix_dir, s1_dir, output):
    """根据 mixture csv 构造 s1 / mix_clean scp 行（含 dev→train 归一化）。"""
...
def main():
    parser = argparse.ArgumentParser(description="生成 VoxCeleb2 scp 列表")
