...
class LauraTSE(AbsESPnetModel):
    ...
    def __init__(
        ...
        # qwen_pretrain_path = "/mnt/users/hccl.local/wwu/Qwen2.5-0.5B/"
        # # 1. 文本 tokenizer（替代原来的 RobertaTokenizer）
        # self.transcript_tokenizer = AutoTokenizer.from_pretrained(qwen_pretrain_path)
        # # 2. 临时加载一次 Qwen2 模型，只为拿 input embedding 权重
        # _tmp_qwen = Qwen2ForCausalLM.from_pretrained(qwen_pretrain_path)
        # orig_emb = _tmp_qwen.get_input_embeddings()           # nn.Embedding
        ...
        # self.text_embed = nn.Embedding(vocab_size, emb_dim)
        # with torch.no_grad():
        ...
        # self.transcript_enc_out_layer = nn.Linear(emb_dim, self.codebook_dim)
        ...
        # self.criterion_ce_normal = torch.nn.CrossEntropyLoss()

        # self.hubert_encoder = HubertModel.from_pretrained("/mnt/users/hccl.local/wwu/.cache/huggingface/hub/models--facebook--hubert-base-ls960")
        # self.hubert_encoder = HubertModel.from_pretrained("facebook/hubert-base-ls960")
        # for key, param in self.hubert_encoder.named_parameters():
        #     param.requires_grad = False

        # self.wavlm_model = WavLMModel.from_pretrained('/mnt/users/hccl.local/wwu/wavlm-base')
        ...
        #     param.requires_grad = False
        # import pdb;pdb.set_trace()
        # self.hubert_proj = nn.Conv1d(768, N, kernel_size=1)  # 特征维度投影
        # self.fusion_layer = nn.Conv1d(2*N, N, kernel_size=1)  # 融合层
    ...
    def transcript_aux_encode(
        ...
        # # 文本 -> BPE ids
        # enc = self.transcript_tokenizer(text_list, return_tensors="pt", padding=True, truncation=True ).to(device)
        # input_ids = enc["input_ids"]          # [B, L]
        # attn_mask = enc["attention_mask"]     # [B, L]

        # # BPE ids -> Qwen embedding
        # text_emb = self.text_embed(input_ids)  # [B, L, 896]
        # # 如需再投到 codebook_dim
        # outs = self.transcript_enc_out_layer(text_emb)  # [B, L, codebook_dim]
        # out_lens = outs.shape[0]
    ...
    def build_llm_io_inference(
        ...
        if visual_sync is not None and codec is not None and codec_lengths is not None:
            # if codec_lengths > visual_sync.shape[1]:
            #     import pdb;pdb.set_trace()
            #     visual_end_feat = torch.zeros((visual_sync.shape[0],1, visual_sync.shape[-1])).to(visual_sync.device)
            #     # visual_sync should align target time step!!!, so add eos!!!
            #     # visual_sync = torch.cat((visual_sync, sos_eos_emb), dim=1).to(visual_sync.device)
            ...
            # else:
            #     # import pdb;pdb.set_trace()
            #     visual_sync = visual_sync[:,1:1+codec_lengths]

            # if codec_lengths.item() >= 98:
            #     import pdb;pdb.set_trace()
            # import pdb;pdb.set_trace()
            ...
            # import pdb;pdb.set_trace()
            # if visual_sync_current.shape[1] != codec_emb.shape[1]:
            #     import pdb;pdb.set_trace()
    ...
    def nll(
        ...
        # import pdb;pdb.set_trace()
        # logits_visual = logits_visual.unsqueeze(2).cuda()
    ...
    def forward(
        ...
        import pdb;pdb.set_trace()
        # transcript_aux = self.transcript_aux_encode(transcript_aux, visual_aux.device) # [B,D]
        ...
        if visual_aux.shape[-1]< aux_lengths.max():
            ...
            # visual_aux = F.pad(visual_aux.transpose(1,2), (0, aux_lengths.max() - visual_aux.shape[1])).transpose(1,2).to(text.device)
        else:
        ...
        # # 5. align on semantic
        # pred_wav = self.codec_model(codec_emb[:,:], run_mod="decode_emb")[2]
        # tgt_wav = self.codec_model(target_emb[:,:], run_mod="decode_emb")[2]
        # min_wav_len = min(pred_wav.shape[-1], tgt_wav.shape[-1])
        # import pdb;pdb.set_trace()
        # pred_ssl_emb = self.wavlm_model(pred_wav[:,:,:min_wav_len].squeeze(1))[0]
        # tgt_ssl_emb = self.wavlm_model(tgt_wav[:,:,:min_wav_len].squeeze(1))[0]
        # import pdb;pdb.set_trace()

        reg_loss, l1_loss, l2_loss = self.calc_reg_loss(codec_emb, target_emb, codec_lengths)
        # ssl_mse_loss = self.calc_ssl_loss(pred_ssl_emb, tgt_ssl_emb)
        av_front_align_mse_loss = self.calc_ssl_loss(visual_aux, tgts_aux_part)

        # 3 loss :111
        # loss = av_front_align_mse_loss
        loss = reg_loss + nll_loss 
        # loss = reg_loss + nll_loss  + av_front_align_mse_loss
        # 10* visual loss + speech ce loss + speech reg_loss
        # loss = reg_loss + nll_loss + 10* nll_visual_loss
        # loss = reg_loss + nll_loss + 5* nll_visual_loss
        # loss = nll_loss + 5* nll_visual_loss
        # loss = nll_loss + 0.1* nll_visual_loss + reg_loss
        # loss = nll_loss  + 10 * ssl_mse_loss
        # loss = reg_loss  
        ...
        stats = dict(
            ...
            # ssl_mse_loss = ssl_mse_loss.detach(),
