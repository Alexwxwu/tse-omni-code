"""把 VoxCeleb2 的 all.scp 路径替换为 FunCodec codec token 路径。

用法:
    python replace_path_vox2_codec.py
    python replace_path_vox2_codec.py --input X --output Y --feat-dir Z
"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="替换 VoxCeleb2 codec token 路径")
    parser.add_argument("--input", default=VOX2["codec_all_scp_train"])
    parser.add_argument("--output", default=VOX2["codec_token_scp_train_avhubert"])
    parser.add_argument("--feat-dir", default=VOX2["codec_token_dir_train_avhubert"])
