...
try:
    ...
    try:
        ...
        # apex 已导入但没有 optimizers 模块或属性，回退到 torch.optim 或 None
        import torch.optim as _optim
        optim_classes.update(
            fusedadam=getattr(_optim, "Adam", None),
            fusedlamb=None,
            fusednovograd=None,
            fusedsgd=getattr(_optim, "SGD", None),
        )
    finally:
        del apex
except ImportError:
    # apex 未安装，使用 torch.optim 回退或保持为 None
    import torch.optim as _optim
    optim_classes.update(
        fusedadam=getattr(_optim, "Adam", None),
        fusedlamb=None,
