"""把 LRS3 的 all.scp 路径替换为 AV-HuBERT 连续特征路径。

用法:
    python replace_path_lrs3_vsr_avhubert_continous_feat.py --kind test
    python replace_path_lrs3_vsr_avhubert_continous_feat.py --input X --output Y --feat-dir Z
"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="替换 LRS3 AV-HuBERT 特征路径")
    parser.add_argument("--input", default=LRS3["codec_all_scp_test"])
    parser.add_argument("--output", default=LRS3["avhubert_feat_scp_test"])
    parser.add_argument("--feat-dir", default=LRS3["avhubert_feat_dir_test"])
