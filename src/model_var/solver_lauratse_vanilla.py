...
class Solver(object):
    ...
    def _reset(self):
        ...
        if self.args.continue_from and self.args.continue_from.lower() != "false":
            self.model = load_model(
                self.model, self.args.continue_from
            )
            ...
            #checkpoint: dict_keys(['epoch' :10 , 'step': 12771, 'model_state_dict', 'optim':'initial_lr': 0.0001; 'lr': 8.8481e-05, 'cv_log', 'scheduler': WarmupLR(warmup_steps=10000), 'new_bob', 'loss': 3.5559195429])
    ...
    def _run_one_epoch(
        ...
        for idx, (mixtures,enrolls,codecs,visual_codecs, vsr_sync_feat) in enumerate(data_loader):  
            visual_sync = vsr_sync_feat.cuda().squeeze(0).float()
            visual_aux = visual_sync[:,:25*2,:]
            ...
            _data_res = {'text': mixtures, 'text_lengths': mixtures_lengths,'aux': enrolls, 'aux_lengths': enrolls_lengths,'codec': codecs, 'codec_lengths': codecs_lengths, 'av_token': visual_codecs}
            ...
            if state == "train":
                ...
                loss, stats, weight = self.model(**_data_res)
                total_loss += stats['nll_loss'].item()
