...
class LlamaLM(nn.Module):
    ...
    def __init__(
       ...
       # ==========================================================
        # 2. 从零直接实例化 LlamaConfig (避免读老路径引入 896 维度的脏残留)
        # ==========================================================
        config = LlamaConfig(
            vocab_size=self.lm_out_voc_size,  # 强制对齐你的平铺音频词表
            hidden_size=512,                  # 缩减 hidden dim 到 512
            num_hidden_layers=12,             # 缩减层数到 12 层 (这里根据你上一条的 Qwen 代码保持 12 层)
            intermediate_size=1536,           # FFN 维度 (512 * 3 = 1536)
            num_attention_heads=8,            # 512 / 8 = 64 (每个 Head 的维度)
            num_key_value_heads=2,            # 开启 GQA（分组注意力），Llama 3 标配
            
            # 以下为 Llama 3 官方推荐的底层默认高稳定性参数
            hidden_act="silu",                # Llama 3 的经典激活函数
            max_position_embeddings=8192,     # Llama 3 默认的长时序支持，对音频帧极其友好
            rms_norm_eps=1e-5,                # Llama 3 标准的归一化精度
            tie_word_embeddings=False
        )
      
        # ==========================================================
        # 3. 实例化全新的空模型 (此时全身上下包含 LayerNorm 都是干净的 512)
        # ==========================================================
        self.codec_lm = LlamaForCausalLM(config)
        # 4. 强制对齐你的输出线性层（从 512 映射到你的平铺大词表）

        # 4. 对齐你的输出层（此时 hidden_size 已经变成 512 了）
        ...
        self.codec_feat_proj_layer = torch.nn.Sequential(
            torch.nn.Linear(128, 512),
            torch.nn.LayerNorm(512),
            torch.nn.ReLU(),
            torch.nn.Dropout(0.1) # 这里的 dropout 比例可以对齐你 YAML 里的 0.1
        )
