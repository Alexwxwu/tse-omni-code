...
from tse_inference_visual_v_conformer_streaming import TSExtraction
...
def cumulate_chunk_est_speech(mix_wav, ref_wav, visual_sync, tse, mel_spec,args, device):
    ...
    while ct <= mix_wav.size(1):
        mix_wav_chunk = mix_wav[:, :ct+hop]
        mask = torch.tensor([mix_wav_chunk.size(1)], dtype=torch.long)
        mix_mel, _ = mel_spec.mel(mix_wav_chunk, mask)
        ...
        visual_sync = visual_sync[:,:int((ct+hop)/16000*25)]

        try:
            output = tse(mix_mel, ref_mel, visual_sync, enroll_setting = args.enroll_setting, enroll_length = args.max_aux_ds, enroll_stage = args.enroll_stage, continual = continual, tgt_scp_path = None) 
        except:
            print(f"Frame {ct} and {ct+hop} of audio len {mix_wav.size(1)} generation ends.")
            break

        continual = output[1].squeeze(0).tolist()
        res.append(output[0]['gen'][:,:,:])

        #     res.append(output[0]['gen'][:,:,:])
        # else:
        #     # import pdb;pdb.set_trace()
        #     res.append(output[0]['gen'][:,:,ct:ct+hop])
        ct +=hop
     
    res = torch.cat(res, dim = -1).to(device)
...
def inference(rank, args):
    ...
    with torch.no_grad(), tqdm.tqdm(scp, desc=f"[inferencing...rank {rank}]") as pbar:
        for paths in pbar:
            ...
            start = time.time()
            output = cumulate_chunk_est_speech(mix_wav, ref_wav, visual_sync, tse, mel_spec, args, ref_mel.device)
            # 1. Inference
