...
def parse_args():
    ...
    parser.add_argument("--Laura", default=0, type=str)
...
def inference(rank, args):
    ...
    mixture_switch_csv = '/mnt/users/hccl.local/wwu/switch_memo_testset/mixture_data_list_3mix_switch_6s_lrs3_test.csv'
    mixture_switch_base_dir = '/mnt/users/hccl.local/wwu/switch_memo_testset/mixture_switch_long_lrs3/test/mix/'

    lrs3_mp4_base_dir = '/mnt/Corpus-Upload/lrs3/mp4/'
    mixture_switch_csv = '/mnt/users/hccl.local/wwu/switch_memo_testset/mixture_data_list_3mix_switch_6s_lrs3_test_1000.csv'
    mixture_switch_base_dir = '/mnt/users/hccl.local/wwu/switch_memo_testset/mixture_switch_long_lrs3_1000samples/test/mix/'

 
    # Inference
    mixture_switch_list = []
    
    with open(mixture_switch_csv) as f:
        mix_switch_lines = f.readlines()
    
     
    total_rtf = 0.0
    with torch.no_grad(), tqdm.tqdm(mix_switch_lines, desc=f"[inferencing...rank {rank}]") as pbar:
        for paths in pbar:
            
            mix_wav_path = mixture_switch_base_dir + '_'.join(paths.split('\n')[0].replace('/',',').split(',')) + '.wav'
            if 'lrs3' in mixture_switch_csv:
                tgt1_video_path = lrs3_mp4_base_dir + paths.split(',')[1] + '/' + paths.split(',')[2] + '.mp4'
                tgt2_video_path = lrs3_mp4_base_dir + paths.split(',')[4] + '/' + paths.split(',')[5] + '.mp4'
                tgt3_video_path = lrs3_mp4_base_dir + paths.split(',')[7] + '/' + paths.split(',')[8] + '.mp4'
            else:
                tgt1_video_path = vox2_mp4_base_dir + paths.split(',')[1] + '/' + paths.split(',')[2] + '/' + paths.split(',')[3]+ '.mp4'
                tgt2_video_path = vox2_mp4_base_dir + paths.split(',')[5] + '/' + paths.split(',')[6]+ '/' + paths.split(',')[7] + '.mp4'
                tgt3_video_path = vox2_mp4_base_dir + paths.split(',')[9] + '/' + paths.split(',')[10]  + '/' + paths.split(',')[11] + '.mp4'
            switch_time = float(paths.split(',')[-1])
            ...
            tgt1_visual_sync =  process_video_with_mask(tgt1_video_path, test_ds_rate, params, mask_type, mask_start, mask_len, partition='test')
            tgt2_visual_sync =  process_video_with_mask(tgt2_video_path, test_ds_rate, params, mask_type, mask_start, mask_len, partition='test')
            ...
            switch_mixture_length = audio.shape[-1]
            tgt_switch_visual = torch.cat((tgt1_visual_sync[:int(switch_time*25)] , tgt2_visual_sync),dim=0)[:int((switch_mixture_length/16000)*25)].to(device)
 
            # # visual_sync[:] = 0
            exp_with_v_corrupt = True
            if exp_with_v_corrupt:
                tgt_switch_visual[int(switch_time*25)+3:] = 0

            #since we only work on visual mode for switching, just simply set ref_mel to mix_mel
            ref_mel = mix_mel
            # 1. Inference
            start = time.time()
             
            if tgt_switch_visual.dim() ==2:
                tgt_switch_visual = tgt_switch_visual.unsqueeze(0).to(device)
             
            output_tuple = tse(mix_mel, ref_mel, tgt_switch_visual, enroll_setting = args.enroll_setting, enroll_length = args.max_aux_ds, enroll_stage = args.enroll_stage, tgt_scp_path = None) 
            if output_tuple[0] == 0:
                print("none!!!")
                continue
            ...
            total_rtf += rtf
            # 2. Save audio
            base_name = Path(mix_wav_path).stem + '_switch_time' + str(switch_time) + ".wav"

            save_path = args.output_dir / base_name
            sf.write(save_path,normalize(output.cpu().numpy(), audio.numpy().squeeze()),samplerate=sr,)
