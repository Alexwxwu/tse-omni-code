...
class LauraTSE(AbsESPnetModel):
    ...
    def build_llm_io_training(
        ...
        for i, codec_len in enumerate(codec_lengths):
            #visual_targets last elem should be  'v_eos', i set it as zero-values
    ...
    def forward(
        ...
        nll, nll_visual, logits, target, target_lengths = self.nll(text, text_lengths, codec[:, :, :self.predict_nq], codec_lengths, av_token)
