...
class LauraTSE(AbsESPnetModel):
    ...
    def __init__(
            ...
            use_lora: bool = False,                     # wrap codec_lm with (frozen) LoRA, kept for ckpt compatibility
            transcript_encoder_path: str = "/mnt/users/hccl.local/wwu/roberta-base",
        ...
        # transcript (text cue) encoder: frozen RoBERTa + gated mix-conditioned prompt
        self.transcript_tokenizer = RobertaTokenizer.from_pretrained(transcript_encoder_path)
        self.transcript_encoder = RobertaModel.from_pretrained(transcript_encoder_path)
        for param in self.transcript_encoder.parameters():
        ...
        # optional (frozen) LoRA wrapper kept for checkpoint compatibility
        # with the former laura_model_only_clean_front_align_omni_lora variant
        ...
        if use_lora:
            ...
            self.codec_lm_lora = LoraModel(self.codec_lm, lora_config, "default")
            for param in self.codec_lm_lora.parameters():
                param.requires_grad = False
