...
class QwenLM(nn.Module):
    ...
    def __init__(
            ...
            use_lora: bool = False,            # wrap codec_lm with peft LoRA (former qwen2_model_omni_av_lora)
            lm_variant: str = "pretrained",    # "pretrained" (Qwen2.5-0.5B) or "small" (custom ~77M config,
                                               #  former qwen2_model_omni_av_small)
            qwen_pretrain_path: str = "/mnt/users/hccl.local/wwu/Qwen2.5-0.5B/",
            codec_config_path: str = "/mnt/users/hccl.local/wwu/lauraTSE_code_refact/codec_config/config.yaml",
            codec_ckpt_path: str = "/mnt/users/hccl.local/wwu/lauraTSE_code_refact/codec_config/model.pth",
    ):
        super().__init__()
        ...
        assert lm_variant in ("pretrained", "small"), f"unknown lm_variant: {lm_variant}"
        if lm_variant == "pretrained":
            self.codec_lm = Qwen2ForCausalLM.from_pretrained(qwen_pretrain_path)
            self.codec_lm.lm_head = nn.Linear(896, self.lm_out_voc_size)
        else:  # "small": custom ~77M config, randomly initialized
            from transformers import Qwen2Config
            config = Qwen2Config.from_pretrained(qwen_pretrain_path)
            config.vocab_size = self.lm_out_voc_size
            config.hidden_size = 512
            config.num_hidden_layers = 12
            config.intermediate_size = 1536
            config.num_attention_heads = 8     # 512 / 8 = 64 per head
            config.num_key_value_heads = 2     # GQA
            self.codec_lm = Qwen2ForCausalLM(config)
            self.codec_lm.lm_head = nn.Linear(512, self.lm_out_voc_size)
        ...
        if use_lora:
            ...
            self.codec_lm = get_peft_model(self.codec_lm, peft_config)
            self.codec_lm.print_trainable_parameters()
        ...
        if lm_variant == "pretrained":
            self.codec_feat_proj_layer = nn.Linear(128, 896)
        else:  # "small"
            self.codec_feat_proj_layer = nn.Sequential(
                nn.Linear(128, 512),
                nn.LayerNorm(512),
                nn.ReLU(),
                nn.Dropout(0.1),
            )
        self.use_visual_aux = use_visual_aux
    ...
    def build_llm_io_training(
        ...
        for sample_idx, text_len in enumerate(text_lengths):
            ...
            if sample_idx in visual_enroll_index:
                ...
                fused_sync_concat = torch.cat((visual_sync, codec_emb),dim=2)
    ...
    def nll(
        ...
        sequence = self.codec_feat_proj_layer(sequence)
        return_tuple = self.codec_lm(inputs_embeds = sequence, output_hidden_states = True, return_dict = True)
        y = return_tuple.logits
    ...
    def forward(
        ...
        idxs = torch.randperm(batch_size).to(text.device)        
        half = batch_size // 2
        audio_enroll_index = idxs[:half]          
        visual_enroll_index = idxs[half:]        

        for text_idx in range(0, len(text)):
            _t = text[text_idx][:text_lengths[text_idx].item()] # [T, D]
            _a = aux[text_idx][:aux_lengths[text_idx].item()] # [T, D]
            _aux_visual = visual_aux[text_idx][:aux_lengths[text_idx].item()] # [T, D]
             
            if text_idx in visual_enroll_index:
        ...
        nll, nll_visual, logits, target, target_lengths = self.nll(text, text_lengths, codec[:, :, :self.predict_nq], codec_lengths, av_token, visual_sync, audio_enroll_index, visual_enroll_index)
    ...
    def decode_codec(
        ...
        for i in range(max_length):
            ...
            seq_input = self.codec_feat_proj_layer(seq_input)
            outputs = self.codec_lm(inputs_embeds = seq_input, output_hidden_states = True, return_dict = True)
            logits = outputs.logits  # 直接获取logits
            pred = logits[:, -1, :]  # 取最后一个位置的logits
