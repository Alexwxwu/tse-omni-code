"""把 VoxCeleb2 遮挡（vocc）scp 路径替换为遮挡 VSR 特征路径。

用法:
    python replace_path_vox2_vsr_vocc_feat.py
    python replace_path_vox2_vsr_vocc_feat.py --input X --output Y --feat-dir Z
"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="替换 VoxCeleb2 遮挡 VSR 特征路径")
    parser.add_argument("--input", default=VOX2["mix_clean_vocc_scp_train"])
    parser.add_argument("--output", default=VOX2["vsr_feat_vocc_scp_train"])
    parser.add_argument("--feat-dir", default=VOX2["vsr_feat_dir_vocc_dev"])
